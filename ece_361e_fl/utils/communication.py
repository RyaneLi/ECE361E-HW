...
def connect(host, port=22, verbose=False):
    ...
    Parameters:
        host (str): The host of the device to connect to.
...
def scp_file(target_host, target_usr, target_pwd, target_path, zip_filename, source_path, verbose=False):
    ...
    Parameters:
        ...
        target_port (int): The port number on the remote host.
    ...
    while True:
        try:
            ...
            if verbose:
                print(f"[!] Error: {e}")
                print(f"[!] Failed to transfer the file. Retrying after 5 seconds...")
            time.sleep(5)
