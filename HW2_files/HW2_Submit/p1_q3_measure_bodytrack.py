"""
Question 3: Bodytrack Benchmark Measurement Script
Run this ON THE ODROID with: sudo python3 p1_q3_measure_bodytrack.py

This script:
1. Sets LITTLE cluster to 0.2GHz, big cluster to 2GHz
2. Runs bodytrack on all 4 big cores (4 threads)
3. Measures power and temperature during execution
4. Saves data to q3_bodytrack_log.txt
"""
...
if __name__ == "__main__":
    ...
    print("Question 3: Bodytrack Benchmark Measurement")
    if args.no_power:
    ...
    # Create log file
    out_fname = 'q3_bodytrack_log.txt'
    ...
    if not args.no_power:
        ...
        try:
            ...
            print("Example: sudo python3 p1_q3_measure_bodytrack.py --no-power")
            exit(1)
    else:
        print("\nSkipping power measurements (power will be recorded as 0.0W)")
    
    # Start bodytrack benchmark on all big cores
    # Mask 0xF0 = binary 1111 0000 = cores 4,5,6,7
    # Arguments: <dataset> <nthreads> <nframes> <n_particles> <n_layers> <do_display> <n_threads>
    # Last arg (4) sets number of threads to use all big cores
    command = "taskset --all-tasks 0xF0 /home/student/HW2_files/parsec_files/bodytrack /home/student/HW2_files/parsec_files/sequenceB_261 4 261 4000 5 0 4"
    print(f"\nStarting benchmark:")
    print(f"  {command}")
    start_time = time.time()
    proc_ben = subprocess.Popen(command.split())
    print("Bodytrack started on cores 4-7 with 4 threads")
    ...
    print("\nBODYTRACK MEASUREMENT COMPLETE!")
