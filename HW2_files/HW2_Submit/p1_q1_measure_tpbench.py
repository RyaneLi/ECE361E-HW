"""
Question 1: TPBench Measurement Script

This script:
1. Sets LITTLE cluster to 0.2GHz, big cluster to 2GHz
2. Runs TPBench on core 4 (first big core)
3. Measures power, temperature, and CPU usage every 0.2 seconds
4. Saves data to q1_tpbench_log.txt
"""
...
def get_cpu_load():
    """Returns the CPU load for each core."""
...
if __name__ == "__main__":
    ...
    print("Question 1: TPBench Measurement")
    if args.no_power:
    ...
    set_user_space()
    set_cluster_freq(0, 200000)    # LITTLE cluster: 0.2 GHz = 200000 KHz
    set_cluster_freq(4, 2000000)   # big cluster: 2 GHz = 2000000 KHz
    print("Frequencies set: LITTLE=0.2GHz, big=2GHz")
    
    # Create log file
    out_fname = 'q1_tpbench_log.txt'
    header = "time\tpower_W\tusage_c0\tusage_c1\tusage_c2\tusage_c3\tusage_c4\tusage_c5\tusage_c6\tusage_c7\ttemp4\ttemp5\ttemp6\ttemp7"
    ...
    if not args.no_power:
        ...
        try:
            ...
            print("Example: sudo python3 p1_q1_measure_tpbench.py --no-power")
            exit(1)
    else:
        print("\nSkipping power measurements (power will be recorded as 0.0W)")
    
    # Start TPBench on core 4 in background
    # taskset mask 0x10 = binary 0001 0000 = core 4
    command = "taskset --all-tasks 0x10 /home/student/HW2_files/TPBench.exe"
    print(f"\nStarting benchmark: {command}")
    proc_ben = subprocess.Popen(command.split())
    print("Benchmark started on core 4")
    
    # Measurement loop - measure while benchmark is running
    print("\nMeasuring power, temperature, and usage every 0.2 seconds...")
    ...
    while proc_ben.poll() is None:  # While benchmark is running
        ...
        if sample_count % 10 == 0:
            print(f"Time: {current_time:6.1f}s | Power: {total_power:5.2f}W | Temp4: {temps[0]:5.1f}°C | Usage_c4: {usages[4]*100:5.1f}%")
    ...
    print("\nTPBENCH MEASUREMENT COMPLETE!")
    print(f"Data saved to: {out_fname}")
    print(f"Runtime: {current_time:.2f} seconds")
    print(f"Total samples: {sample_count + 1}")
    print("\nNext steps:")
    print(f"1. Transfer this file to your local machine:")
    print(f"   scp student@<odroid-ip>:{out_fname} ./")
    print(f"2. Run the plotting code in p1.ipynb on your local machine")
